...
aws_counting_aged_ebs_snapshot() {
...
if ((AWS_SNAP_COUNT>0)); then
    ...
    printf "You have %s aged EBS snapshots (more than %s day since creating), which cost you approximately ${GREEN}\$%.2f${RESET} per month \n" "$AWS_SNAP_COUNT" "$AWS_EBS_SNAP_DAYS" "$AWS_SNAP_SUMM"
