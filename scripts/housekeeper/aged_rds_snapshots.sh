...
aws_counting_aged_rds_snapshots() {
  ...
  if ((AWS_DB_SNAP_COUNT>0)); then
    ...
    printf "You have %s aged RDS snapshots (more than %s day since creating),     \
          which cost you approximately ${GREEN}\$%.2f${RESET} per month \n" "${AWS_DB_SNAP_COUNT}" \
          "${AWS_DB_SNAP_DAYS}" "${AWS_DB_SNAP_SUMM}"
    printf "These snapshots are:\n"
    printf "%s\n" "${AWS_DB_SNAP_LIST}"
  else
    printf "${GREEN}You have no aged RDS snapshots${RESET}"
  fi
  COST=$(printf "%.0f\n" ${AWS_DB_SNAP_SUMM})
  reply=(${COST} "$@")
}
